...
int main(int argc, char** argv) {
    ...
    Dim dim3 (init3, sizeof(init3) / sizeof(float));
    assert(dim3.sum() == 0);

    float init4[] = {3, -3, 0.5, -8, 0.11, 1, -0.85};
    Dim dim4 (init4, sizeof(init4) / sizeof(float));
    assert(dim4.sum() == 0.5);

    float init5[] = {3, -3, -0.5, 8, 0.11, 1, -0.85};
    Dim dim5 (init5, sizeof(init5) / sizeof(float));
    assert(dim5.sum() == 0);

    float init6[] = {-3, 3, 0.5, 8, 0.12, 1, -0.85};
    Dim dim6 (init6, sizeof(init6) / sizeof(float));
